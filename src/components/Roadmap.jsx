...
const Roadmap = () => {
    ...
    return (
        <section className='w-full h-screen py-20 bg-myred text-black relative'>

            <img src="/aboutline.webp" alt="Lines" className="absolute right-0 -top-40" />
                        ...
                        {phase.map((items, index) => {
                            return (
                                <div className={`h-auto bg-white flex flex-col items-center py-10 relative transition-all ease-in-out duration-300 ${current === index ? "shadow-glow2 opacity-100" : "opacity-50"}`} key={index} style={current === index ? { flexGrow: "3" } : {}} onMouseEnter={() => { onEnter(index) }} onMouseLeave={onLeave}>
                                    ...
                                    <div className='mt-4 flex flex-col gap-y-4  transition-all ease-in-out duration-300 origin-top font-poppins' style={current === index ? { scale: "1" } : { scale: "0" }}>
                                        {items.label}
                                    </div>
                                </div>
                            )
